...
class XLMIMO_System:
    ...
    def calculate_secrecy_rate(self, H_LUE, H_EUE, W, epsilon, debug=False):
        ...
        for k in range(self.K):
            for e in range(self.E):
                signal_k = epsilon * self.P_b * np.abs(H_LUE[k, :] @ W[:, k])**2 # Signal power at Bob
                noise_an_k = AN_power * np.linalg.norm(H_LUE[k, :] @ V_0)**2 # IAN leakage at Bob (should be 0)
                if debug:
                    print(f"Signal Power at Bob: {signal_k:.4e} W | IAN Leakage at Bob: {noise_an_k:.4e} W | N")
                R_k = np.log2(1 + signal_k / (noise_an_k + self.sigma2))

                signal_e = epsilon * self.P_b * np.abs(H_EUE[e, :] @ W[:, k])**2 # Signal power at Eve
                noise_an_e = AN_power * np.linalg.norm(H_EUE[e, :] @ V_0)**2 # IAN leakage at Eve (should be >0)
                R_e = np.log2(1 + signal_e / (noise_an_e + self.sigma2))
                if debug:
                    print(f"Signal Power at Eve: {signal_e:.4e} W | IAN Leakage at Eve: {noise_an_e:.4e} W | N")
...
if __name__ == "__main__":
    xl_mimo = XLMIMO_System(K=1, E=1, bob_loc=np.array([10, 0, 10]), eve_loc=np.array([12, 0, 10]), config=Config())
    secrecy_rate = xl_mimo.run_alternating_optimization()
